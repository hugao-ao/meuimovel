...
function calcularFinanciamento(dados) {
    ...
    if (dados.sistemaCorrecao !== 'fixo') {
        parcelas = aplicarCorrecaoMonetaria(parcelas, valorFinanciado, dados.sistemaCorrecao, dados.sistemaAmortizacao);
    }
 ...
 * @param {string} sistemaAmortizacao - Sistema de amortização utilizado
 * @returns {Array} - Array de parcelas com correção aplicada
 */
function aplicarCorrecaoMonetaria(parcelas, valorFinanciado, tipoCorrecao, sistemaAmortizacao) {
    ...
    // Para o SAC, calcular a amortização constante uma única vez
    const amortizacaoConstanteSAC = sistemaAmortizacao === 'sac' ? valorFinanciado / parcelas.length : 0;
    ...
    for (let i = 0; i < parcelas.length; i++) {
        ...
        if (sistemaAmortizacao === 'sac') {
            ...
            novaAmortizacao = amortizacaoConstanteSAC;
            const taxaJurosMensal = parcela.juros / (parcela.saldoDevedor + parcela.amortizacao);
            ...
            novaPrestacao = saldoDevedor * (taxaJurosMensal * Math.pow(1 + taxaJurosMensal, prazoRestante)) / (Math.pow(1 + taxaJurosMensal, prazoRestante) - 1);
            const juros = saldoDevedor * taxaJurosMensal;
            novaAmortizacao = novaPrestacao - juros;
        } else {
...
function simularAmortizacaoExtraordinariaPrazo(dadosFinanciamento, valorAmortizacao, parcelaAtual) {
    ...
    if (novosDados.sistemaCorrecao !== 'fixo') {
        novasParcelas = aplicarCorrecaoMonetaria(novasParcelas, novoSaldoDevedor, novosDados.sistemaCorrecao, novosDados.sistemaAmortizacao);
    }
...
function simularPortabilidade(dadosAtuais, dadosNovos) {
    ...
    // Aplicar correção monetária se necessário
    if (novosDados.sistemaCorrecao !== 'fixo') {
        novasParcelas = aplicarCorrecaoMonetaria(novasParcelas, saldoDevedorAtual, novosDados.sistemaCorrecao, novosDados.sistemaAmortizacao);
    }
